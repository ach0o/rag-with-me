...
```
질문 확장 → 검색 (서브쿼리별) → 중복 제거 → 컨텍스트 평가 →[충분]→ 답변 생성 → 완료
                                                  ↓
                                                [부족]
                                                  ↓
                                              질문 재구성 → 검색 (최대 2회 재시도)
...
복잡한 질문은 2-4개의 서브쿼리로 분해하여 더 넓은 범위의 문서를 검색합니다. 청크는 중복 제거 후 원래 질문 기준으로 리랭킹됩니다. 평가 LLM이 컨텍스트 품질을 판단하고, 부족하면 질문을 재구성하여 재검색합니다. 단순 선형 질의 경로(`QueryUseCase`)도 폴백으로 사용 가능합니다.
